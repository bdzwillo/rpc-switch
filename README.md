...
Setup:
```
$ mkdir -p etc                              # not in the tree: .gitignore has /etc
$ cp doc/examples/config.pl etc/config.pl   # one plain listener on port 6551
$ cp doc/examples/methods.pl etc/methods.pl # the foo.* to bar.* mapping
```
