...
# channel information is checked
{
	my $c = client();

	my $r = $c->call('foo.power', {},
		rpcswitch => {vcookie => 'notme', vci => 'x'});
	is $r->{error}->{code}, -32004, 'a bad cookie is refused';

	$r = $c->call('foo.power', {},
		rpcswitch => {vcookie => 'eatme', vci => 'nosuchchannel'});
	is $r->{error}->{code}, -32005, 'an unknown channel is refused';
}
