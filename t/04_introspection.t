...
# get_clients and get_workers list who is connected
{
	my $w = $sw->connect;
	$w->hello('theEmployee');
	$w->announce('bar.add', workername => 'adder');
	...
	is_deeply $workers->{adder}, ['bar.add'], 'get_workers lists the method';
}

# get_stats counts connections and calls
{
	my $w = $sw->connect;
	$w->hello('theEmployee');
	$w->announce('bar.add');
	...
	my $c = $sw->connect;
	$c->hello('deKlant');
	# a notification is counted too, and needs no worker reply
	$c->notify('foo.add', {a => 1, b => 2});

	my $stats = $c->call('rpcswitch.get_stats')->{result}[1];
	cmp_ok $stats->{clients}, '>=', 2, 'get_stats counts the clients';
	cmp_ok $stats->{connections}, '>=', $stats->{clients},
		'get_stats counts all connections so far';
	cmp_ok $stats->{chunks}, '>', 0,
		'get_stats counts the messages handled';
	is $stats->{methods}->{'foo.add'}, 1,
		'get_stats counts the calls per method';
}
