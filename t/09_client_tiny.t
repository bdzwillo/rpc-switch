...
# client module for both ends at once
#
my $mon = $sw->connect;
$mon->hello('deKlant');

ok(TestSwitch::wait_for(sub { $mon->call('rpcswitch.get_workers')->{result}[1]->{tinyworker} }, 20), "test tiny worker announce");
is_deeply($mon->call('rpcswitch.get_workers')->{result}[1]->{tinyworker}, ['bar.add'], "test tiny worker method");
