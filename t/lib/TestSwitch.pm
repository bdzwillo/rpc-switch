...
BEGIN {
	my @missing = grep { not eval "require $_; 1" }
		qw(CBOR::XS JSON::MaybeXS LMDB_File Mojolicious MojoX::LineStream
			MojoX::NetstringStream POSIX::RT::MQ Ref::Util);
	Test::More::plan(
...
# acls passed on to the workers
$visible_acl = [qw( klant )];
EOF
...
sub _write_passwd {
	my ($self) = @_;
	# crypt(3) sha-256, one of the formats Auth::Password accepts
